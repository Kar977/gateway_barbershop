...
from gateway.gateway.auth.auth0_client import verify_employee_role, verify_business_owner_role  # dostosuj import!
...
def override_employee_role():
    async def dummy_dependency(request: Request):
        return True  # lub None — zależnie, co potrzebujesz
...
def mock_request(monkeypatch):
    def _mock_request(mocked_response: dict):
        ...
        monkeypatch.setattr(
            "gateway.gateway.routers.users_manager.organization.send_request_to_service",  # <- popraw ścieżkę jeśli inna
            async_mock
        )
...
def mock_employee_request(monkeypatch):
    def _mock_request(mocked_response: dict):
        ...
        monkeypatch.setattr(
            "gateway.gateway.routers.employees_manager.employees.send_request_to_service",  # <- popraw ścieżkę jeśli inna
            async_mock
        )
    return _mock_request
...
def mock_customer_request(monkeypatch):
    def _mock_request(mocked_response: dict):
        ...
        monkeypatch.setattr(
            "gateway.gateway.routers.customer_manager.workday.send_request_to_service",  # <- popraw ścieżkę jeśli inna
            async_mock
        )
    return _mock_request